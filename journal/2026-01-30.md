...
- Full recording → captures BOTH my daughter's question AND parent's repeat ✅
- 38s sample → captures my daughter's question, SKIPS parent's repeat ❌
...
# Later Session: LLM Comparison Review + Prompt Iteration

## Session Focus

Review first comparison results, debug prompt issues, iterate until models comply and reason correctly.

## Problems Found

**Problem 1: Models reassigning existing labels**

Original prompt included Example 4 ("single word reassignment") and rule "A single word may need reassignment even mid-phrase." Models interpreted this as permission to change confident assignments.

DeepSeek's first run: 12 "other changes" — changed things that were already correctly assigned.

**Problem 2: Qwen3 format non-compliance**

Across multiple attempts, Qwen3 refused to follow REASONING/CORRECTED format. Instead produced elaborate line-by-line analysis. Even explicit "you must follow this exactly" with complete example response didn't work.

Irony: Qwen3's *reasoning* was actually correct ("why do the Fondos..." is the child's question). It just wouldn't output in our format.

## Prompt Evolution

**v1 (original):** Four examples, asked for reasoning then CORRECTED:
- Qwen3: Format fail
- DeepSeek: 12/13 resolved, but 6 "other changes"

**v2:** Added "Do NOT change existing assignments", removed Example 4
- Qwen3: Still format fail
- Gemma: 0 other changes ✓ but wrong reasoning
- DeepSeek: Timeout

**v3:** Explicit REASONING/CORRECTED headers with complete example response
- Qwen3: Still format fail (stubborn)
- DeepSeek: Followed format but changed existing assignments
- Gemma: Followed format, wrong reasoning

**v4 (final):** Simplified — removed reasoning requirement entirely
```
Output ONLY the corrected transcript with all <spk:?> resolved. 
No explanation, no formatting, just the transcript.
```

## Final Results

| Model | Time | Format | Respected existing | Correct reasoning |
|-------|------|--------|-------------------|-------------------|
| DeepSeek R1 8B | 32s | ✓ | ✓ | ✓ |
| Qwen3 8B | 131s | ✓ | ✓ | ✗ |
| Gemma 3 12B | 23s | ✓ | ✗ | partial |

**Winner: DeepSeek R1 8B**

## Key Insight

DeepSeek correctly identified the conversational pattern:
- Parent: "what is the question?"
- Child: "why do the Fondos and the Goros want to be king?"

Other models assigned the child's question to the parent, misunderstanding who was asking.

## Learnings

1. **Simpler prompts work better** for format compliance
2. **Chain-of-thought not always necessary** — DeepSeek reasoned correctly without explicit reasoning output
3. **Example 4 was actively harmful** — taught models to change things they shouldn't
4. **Test format compliance before reasoning quality** — no point evaluating wrong output

## What's Next

Integrate DeepSeek R1 8B into pipeline as post-alignment correction step. Create ground truth for validation.
