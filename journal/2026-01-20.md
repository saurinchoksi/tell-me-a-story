...
Tell-Me-A-Story: A local story capture pipeline. Record and input bedtime stories with my daughter. Output speaker-labeled transcripts. Build a searchable story bible over time. Gain insights into her storytelling and her characters, worlds, themes, etc.
...
## Session 4: Testing, Git, and Project Structure

**Wrote tests for transcribe.py (`tests/test_transcribe.py`):**
- `test_save_transcript_creates_file` — Unit test with mock data, verifies file format
- `test_save_transcript_handles_empty_segments` — Edge case for empty input
- `test_transcribe_returns_expected_structure` — Integration test (marked `@pytest.mark.slow`)
- `test_transcribe_segments_have_timestamps` — Integration test verifying segment structure

**Set up pytest configuration (`pyproject.toml`):**
- Registered `slow` marker for integration tests
- Set `testpaths = ["tests"]`
- Now can run `pytest -m "not slow"` for fast feedback (~0.8s) or full suite (~8s)

**Initialized Git:**
- `git init`
- First commit ready (haven't pushed—no GitHub remote yet)

**Designed file naming convention:**
- Audio files: `YYYYMMDD-HHMMSS.m4a`
- Timestamp-based, no metadata in filename
- Device ID deferred—will be added at ingest when multiple devices exist
- Manifest/database will track meaning (story title, participants, themes)
- Test file: `00000000-000000.m4a` (parses like real data, zeros signal test fixture)

**Reorganized project structure:**
- `data/` → `stories/audio/` and `stories/transcripts/`
- Matching filenames (minus extension) correlate files across stages
- Updated `.gitignore` and all test paths

**Key learnings:**
- Run pytest from project root (paths are relative)
- TOML is modern config format—`pyproject.toml` consolidates all tool config
- Filename should handle identity; manifest handles meaning
- iOS Voice Memos doesn't embed device info—device tagging happens at ingest
...
Diarization. Use pyannote.audio to separate speakers in the audio, then align with Whisper's timestamped segments.
