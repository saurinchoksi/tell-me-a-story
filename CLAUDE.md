...
python src/detect.py --judge                  # + offline M9b LLM judge (loads Gemma; slow)
```
...
- **M9b offline LLM judge** (`src/detectors/name_consistency_judge.py`) — recovers improvised names that are also dictionary words (Bibi, Bacchus). `run(session_dir, judge=...)` hands the all-common clusters to Gemma-4 E4B (the M9b eval winner, 1.00/1.00; see `emp/src/judge_m9b.py` + `emp/writeup/name-consistency-eval.html`), lifting recall to 0.86 @ 0.88 precision. **Offline only** — too slow for the live API (a ~30s model load can't sit in a GET); run it with `python src/detect.py --judge`. It runs the model in a fresh subprocess of the project venv (a clean process so a pyannote MPS allocation can't block the Gemma load; `mlx-vlm` is installed in the one venv alongside everything else). A judge run survives normal Monitor viewing (fingerprint unchanged); a transcript change reverts it to code-only until you re-run `--judge`.
- **`src/detectors/story_names/`** — `m9c-canon` (v0.4.0-experimental, `offline_only`): the per-story canon-name auditor on Qwen3.5-4B. Per story it recognizes the world from the name list, generates a cast, runs an **order-robust judge** — the judge's catch on a borderline name flips with the *order* of the name list, so it votes across **K=7 deterministic shuffles and keeps a wrong-spelling caught in ≥3** (`judge_names_voted` in `_qwen35.py`) — unioned with exact Double-Metaphone matching against the cast, dictionary-gated, then **surfaces only confident catches** (the suggested spelling shares a DM code with the heard token, which drops the judge's over-maps of *invented* names onto canon, e.g. an unseen Thomas recording's "Pataki"→"Paddy"). Held-out Mahabharata **5/9 recall, 5/6 precision, deterministic**. `config_fingerprint` hashes the voting params + the three prompts so a tuning change re-scans. The Qwen3.5 runtime recipe (it's a VLM → `mlx_vlm`; reasoning model → `enable_thinking=False`; plain text, never JSON) lives in `src/qwen35.py`; the Gemma `_worker.run` stays as a baseline; tuned in `emp/src/tune_judge_vote.py`.
- **M9b↔M9c dedup (view-time, `api/routes/detections.py` `_apply_canon_dedup`):** M9b defers to M9c on canon names, **cluster-aware** — a whole M9b spelling-cluster defers when M9c owns any token of it, so a canon name never shows split across the inconsistency (M9b) and canon (M9c) sections. The three name detectors render in one **unified grouped layout** (one header per name, occurrences nested) in `ui/src/pages/SessionDetections.tsx`.
