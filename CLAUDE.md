...
python src/detect.py --judge                  # + offline M9b LLM judge (needs venv-mlx-vlm)
```
...
- **M9b offline LLM judge** (`src/detectors/name_consistency_judge.py`) — recovers improvised names that are also dictionary words (Bibi, Bacchus). `run(session_dir, judge=...)` hands the all-common clusters to Gemma-4 E4B (the M9b eval winner, 1.00/1.00; see `emp/src/judge_m9b.py` + `emp/writeup/name-consistency-eval.html`), lifting recall to 0.86 @ 0.88 precision. **Offline only** — too slow for the live API (a ~30s model load can't sit in a GET); run it with `python src/detect.py --judge`. It subprocesses to the **mlx-vlm venv** (the Gemma-4 build won't load under the main venv's mlx-lm): create once with `python -m venv venv-mlx-vlm && ./venv-mlx-vlm/bin/pip install 'mlx-vlm==0.5.0' metaphone` (gitignored). A judge run survives normal Monitor viewing (fingerprint unchanged); a transcript change reverts it to code-only until you re-run `--judge`.
...
- **Runner:** `python src/detect.py` (see Commands) force-runs all registered detectors; add `--judge` for the M9b LLM layer; `process_inbox.py` runs the registered (code-only) detectors on each new session.
- **Freshness:** each section stamps a `transcript_fingerprint` (sha256 of `transcript-rich.json`) and a `config_fingerprint` (sha256 of the detector's config — the roster for M9a; `None` for configless detectors). The detection API routes call `ensure_fresh_detections()` (`src/detectors/base.py`) on every view — any section missing or fingerprinted against an older transcript OR config is re-run inline (detectors are pure code, ~ms/session). Viewing the Monitor after `--re-enrich` or a roster edit re-scans automatically; no manual step, no server restart (the roster cache is keyed by content hash).
