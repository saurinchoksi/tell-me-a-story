...
4. **pipeline.py** — Orchestrates all stages:
   ...
   - `enrich_transcript()` — runs the interpretive enrichment passes:
     ...
     - **Story segmentation** (moved BEFORE name fixing 2026-07-02) — `_stories` regions via Qwen3.5; cached (`model_cache.py`).
     - **Name fixing (`namefix.py`) — the world-blind normalizer's replacement, SHIPPED 2026-07-02.** (The old `normalize.py`/Qwen3-8B pass was removed 2026-07-01 after mapping the child's "Fondos"/Pandavas onto "Bhishma" — confirmed by ear.) Namefix corrects by LISTENING again, not guessing from spelling: per story it recognizes the world from the name list (abstains on invented worlds → does nothing), generates the world's cast with SPLIT prompts — characters AND groups/families (`worldcast.py`; "characters" alone drops Pandavas/Kauravas) — then re-transcribes a ~3.5s clip around each name candidate with the cast in Whisper's `initial_prompt` and applies a gated decision: nearest-non-ordinary word at the target offset, ordinary re-decodes never map to cast, a blind token that's a real English word (frequency list `commonwords.py`/`data/common-words.txt`, NOT Webster's — see the bando/jami/Kauravas ghosts) is never auto-overwritten → it QUEUES. Auto-fixes apply via `corrections.apply_corrections` (id/word-count preserving). Queue → `sessions/<id>/pending-name-corrections.json`. Two sequential model subprocesses (Qwen then Whisper). Validated: clean-room fresh run scored 32/35 on the by-ear key (24 exact, 0 wrong writes); rollout held the invented-copy Thomas night entirely in the queue (his 6/6 rejections). `namefix_session.py` = dry-run/apply/demote for existing sessions (shares the pipeline cache so --write applies exactly the reviewed decisions); `revert_normalization.py` reverts the OLD normalizer's damage via `_original` trails.
     - **The review loop:** `GET /api/name-corrections` (batch rollup, world → name), bless/reject POSTs (per-occurrence bless supported; partial bless never writes the blessing store), UI at `/name-review` (in-place ▶ audio per occurrence + the occurrence's sentence with the judged word highlighted). **The blessing store (`data/worlds/<slug>.json`, via `worlddict.py`) is deliberately EMPTY (2026-07-02) — writes happen ONLY through Choksi's explicit bless action; its redesign + rename is a queued project (emp/emp.md).**
...
- **`emp/results/visuals/<sid>/name-truth.json`** — **the canonical by-ear name KEY per session** (settled 2026-07-02; the older validation-notes name rows are historical, not a scoring source). Marked via `emp/src/name_truth.py --serve`; every save stamps `_transcript_fingerprint` + `_marked_at`, and `score_canon_heldout.py` warns on unstamped/stale keys. **Keys NEVER feed the fixer** — they only score it (the firewall; see memory name-key-rules).
- **`sessions/<id>/pending-name-corrections.json`** — the namefix bless queue (+ `_blessed`/`_rejected` audit lists). Written by the namefix stage and the bless/reject endpoints; carries `config_fingerprint` + `transcript_fingerprint`.
