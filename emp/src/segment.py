#!/usr/bin/env python3
"""Stage-0 story segmenter — split a recording into its stories (EMP).
...
"Code proposes, model confirms," the same shape as the name detectors:
  ...
  1. PRE-FILTER (cheap code, high recall) — verbal cues + time-gaps + structural
     markers + recording-edge sentinels propose candidate boundary zones. Tuned
     for recall: a candidate near every true boundary; false ones are cheap.
     (Validated at 14/14 boundary recall — see segment_prefilter.py.)
  2. READER pass 1 (local LLM) — per candidate zone, the model extracts the
     boundary EVENTS in a small window: story_start / story_end / nothing. It
     rejects the false zones (filler loops, mid-story pauses) and places the
...
venv-mlx-vlm; family content never leaves the machine. Output carries transcript
quotes, so it lands in gitignored emp/results/visuals/.
...
PRED_OUT = ROOT / "emp" / "results" / "visuals" / "segmentation-pred.json"
RAW_OUT = ROOT / "emp" / "results" / "visuals" / "segmentation-pred.raw.json"
...
def load_truth():
    sess = json.loads(SIDECAR.read_text())["sessions"]
    return {sid: sess[sid]["stories"] for sid in SESSIONS}
...
def segment_session(sid, gen, show_only=False, use_global=True, use_refine=False):
    ...
    if show_only:
        print(f"\n===== {SESSIONS[sid]} ({sid}) — {len(zones)} zones =====")
        for z in zones:
            positions, elide = build_window(segs, z)
            print(f"\n--- zone pos {z['positions'][0]}-{z['positions'][-1]}  [{', '.join(sorted(z['signals']))}] ---")
            print(render_window(segs, positions, z["sig_at"], elide))
        return None
    ...
    print(f"  {SESSIONS[sid]:12s}: {len(zones):2d} zones -> {n_events:2d} events -> {merge_note} stories  "
          + " | ".join(f'{s["start_id"]}-{s["end_id"]} [{s["world"]}]' for s in stories))
    return {"name": SESSIONS[sid], "n_zones": len(zones), "n_events": n_events,
            "n_regions_pre_merge": n_pre, "stories": stories}, raw_log


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("session", nargs="?", help="one session id; default = all")
    ...
    PRED_OUT.write_text(json.dumps(pred, indent=2, ensure_ascii=False))
    RAW_OUT.write_text(json.dumps(raws, indent=2, ensure_ascii=False))
    print(f"\nwrote {PRED_OUT.relative_to(ROOT)}  (+ raw model output to {RAW_OUT.name})")
