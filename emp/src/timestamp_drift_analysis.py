...
Examples:
    python emp/src/timestamp_drift_analysis.py                 # all five sessions
    python emp/src/timestamp_drift_analysis.py --session 20260129-204404 --story-end seg:594
    python emp/src/timestamp_drift_analysis.py --session 20251210-203654 --story-end 9:37.12
"""
...
def build_summary(results, noise_pct, margin):
    ...
    lines.append("Whisper guesses each word's start/end time instead of measuring it, so the times "
                 ...
                 "`emp/src/timestamp_drift_analysis.py`.")
    lines.append("")
    ...
    lines.append("- Per-session visual breakdowns: `emp/results/visuals/<id>/timestamp-drift.html` and "
                 "`emp/results/visuals/<id>/drift-to-check.html` (local only — they contain transcript text).")
    return "\n".join(lines) + "\n"
...
def process(session, args):
    ...
    sdir = analysis["sdir"]
    html_path = _visdir(sdir) / "timestamp-drift.html"
    html_path.write_text(render_html(session, analysis, story_end))
    check_path = _visdir(sdir) / "drift-to-check.html"
...
def main():
    ...
    p.add_argument("--summary-out",
                   default="emp/results/timestamp-drift/summary.md",
