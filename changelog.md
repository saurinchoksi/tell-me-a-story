...
Every word in a transcript carries a start and end time, but the transcriber guesses those rather than measuring them, so they drift — and by ear you can only catch the worst few. A new pass weighs each word's claimed moment against the actual sound across all five reviewed recordings and flags the clear misses: a near-silent spot at the claimed time with the real word audible right beside it. The longest recording held around sixty such words against the dozen-or-so a person could catch by ear, confirming the true rate is several times what hand-counting shows — while quiet-but-real speech and isolated noises are held aside as separate problems so the count stays honest. Stretches where the transcriber gets stuck repeating a filler word are left out too: that's already counted as its own kind of failure, so its fake words shouldn't masquerade as timing errors. An early attempt to also flag words pinned to the wrong voice was dropped once listening showed it was catching the speaker detector splitting one storyteller's character voices into several people, not a timing problem at all.
