...
## 2026-06-19 — The canon name-checker now runs on a smarter model, and it works

The check that catches a known character spelled wrong — Bhishma heard as "Bishma," Karna as "Garn" — went from barely working to working, and the fix was a better model, not more code. The old small reader caught about 1 of 11 real misspellings on the held-out Mahabharata recording; a newer model that fits the same memory budget, asked the original simple question, caught 8 of 11 — including the badly-garbled name we'd written off as the audio floor. The story-splitting step moved to the new model too (cleaner boundaries, and it now recognizes worlds the old one missed, like Thomas the Tank Engine), and every existing recording was re-split and re-checked so the monitor reflects the change everywhere — three recordings now surface misspelled characters and the made-up stories correctly show none. The lesson worth keeping: a capable model wants the simplest prompt, and the exact wording that passed testing is the wording that has to ship — a single stray phrase ("names may be misspelled") quietly switched off world-recognition until we removed it.

## 2026-06-17 — Canon name-checker stops flagging everyday words as misspelled characters

The check that catches misspelled characters from a known story — a Mahabharata hero, say — had a noisy flip side: on the held-out Mahabharata recording it treated ordinary words like "arrows" and "wars" as misspellings of a character it had invented called "Arrow," turning 2 real catches into 24 flags. It now ignores a word the dictionary already knows unless the story actually uses it as a name, trimming those 24 down to the 2 true ones. The careful part was keeping the real names: the word list on this machine includes much of the mythology itself (Krishna, Drona, even Dhritarashtra), so "it's in the dictionary" can't mean "it's ordinary" on its own — a word is dropped only when the dictionary knows it and the story never writes it as a capitalized name. That's what keeps a genuinely misspelled hero caught while still discarding "arrows."

## 2026-06-17 — Bedtime stories now recognize their world from the whole story

The step that decides whether a recording comes from a known world (like the Mahabharata) or is just made up was reading only about a dozen scattered lines of each story and leaning on a few hard-coded example worlds — so on a long Mahabharata recording it never saw the telling names, called the whole thing "made-up," and that quietly switched the canon-name check off and let every misspelled name through. It now reads the whole story and uses a general prompt that draws on what the model already knows, with no built-in list of our own recordings' worlds. To prove the change I wrote a hundred fabricated bedtime stories — half from real worlds like Star Wars and He-Man, ranging from obvious to barely-hinted, half made up — and scored the old wording against new ones: the best new prompt went from about two-thirds right to four-fifths, and largely stopped inventing a world for stories that were actually made up. The honest surprise, caught only by testing on the held-out Mahabharata recording the tuning never touched: the bigger cause of that miss was the dozen-line sampling, not the wording — fed the whole story, even the old prompt recognized the Mahabharata. So both fixes ship, and the recording the monitor used to miss now gets labelled correctly. One limit worth naming: the small on-device reader still struggles to place less-famous shows (a preschool cartoon, an old book series) from a few garbled names — a knowledge limit of a small model, not something more wording will fix.

## 2026-06-17 — Canon names show under canon only, and every recording is now fully scanned

When the canon reader recognizes a known name spelled wrong (like the Mahabharata's "Pandavas"), that name now shows only under the canon check, not also doubled under the made-up check where it used to appear as "inconsistent." The de-duplication happens when you view a recording, and it safely does nothing when the reader didn't recognize the story's world — so a recording where the reader missed the canon keeps those catches under the made-up check rather than losing them. Separately, ran the full detector set on every existing recording — the canon reader and the made-up-name judge — so they all now show what a freshly-added recording would, which is the point: you can read the monitor and trust it reflects the current checks everywhere. One honest finding surfaced by doing this: the small on-device canon reader recognized the world on the recordings it was built on (it caught the Mahabharata names in one, and correctly stayed silent where a show's names were spelled right) but misread the held-out Mahabharata recording as "made-up" and caught nothing there — so on that one, the canon errors are still caught by the made-up check instead. The plain code checks stayed dependable throughout.

## 2026-06-17 — The Monitor's three name checks now read cleanly

Fixed three things that made the Monitor's name flags confusing. Pages with a canon-name flag were blanking out — the screen only knew two of the three flag shapes and tripped over the third; it now handles all three (and any future one) without crashing. The made-up-name check was also doing two jobs badly: it re-flagged the family name that the family-name check already owns, and it flagged contraction words like "you're" as if they were names. It now leaves family names to their own check and ignores contractions, so the three checks finally mean three distinct things — family, made-up, and canon. Biggest change: the made-up check **stopped flagging correct spellings**. A made-up name has no answer key, so it used to flag every occurrence of any name spelled more than one way — which meant a name spelled right 46 times with one stray typo lit up 47 times. Now it treats the spelling used most often in a recording as the intended one and flags only the odd ones — so you see the single typo to fix, not the 46 correct spellings. When no spelling clearly wins (a name scattered several ways with no favourite), it still shows them all, since there's nothing to call correct. That "most common wins" is only ever a within-recording call about what to surface — never a claim about the one true spelling across recordings, which is a separate, future job.
