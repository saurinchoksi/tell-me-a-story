...
# --- num_speakers hint tests ---

def _run_pipeline_with_num_speakers(num_speakers_kwarg):
    """Helper: run pipeline with given num_speakers kwarg, return (result, diarize_mock)."""
    fake_transcript = copy.deepcopy(_FAKE_TRANSCRIPT)
    enrichment_processing = [
        {"stage": "diarization_enrichment", "model": "pyannote/speaker-diarization-community-1", "status": "success"},
        {"stage": "gap_detection", "gaps_found": 0, "status": "success"},
    ...
    with contextlib.ExitStack() as stack:
        ...
        result = run_pipeline(
            "/sessions/20260301-120000/audio.m4a",
            verbose=False,
            **num_speakers_kwarg,
        )
    return result, mocks["diarize"]


def test_pipeline_num_speakers_default_is_2():
    """Default num_speakers=2 is forwarded to diarize and stored in metadata."""
    result, diarize_mock = _run_pipeline_with_num_speakers({})
    diarize_mock.assert_called_once_with("/sessions/20260301-120000/audio.m4a", num_speakers=2)
    assert result["diarization"]["_num_speakers_hint"] == 2


def test_pipeline_num_speakers_explicit_override():
    """Explicit num_speakers=3 is forwarded to diarize and stored in metadata."""
    result, diarize_mock = _run_pipeline_with_num_speakers({"num_speakers": 3})
    diarize_mock.assert_called_once_with("/sessions/20260301-120000/audio.m4a", num_speakers=3)
    assert result["diarization"]["_num_speakers_hint"] == 3


def test_pipeline_num_speakers_none_omits_hint():
    """num_speakers=None is forwarded to diarize and stored as None in metadata."""
    result, diarize_mock = _run_pipeline_with_num_speakers({"num_speakers": None})
    diarize_mock.assert_called_once_with("/sessions/20260301-120000/audio.m4a", num_speakers=None)
    assert result["diarization"]["_num_speakers_hint"] is None
