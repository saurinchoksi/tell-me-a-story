...
from normalize import _parse_llm_corrections, llm_normalize, DEFAULT_PROMPT
...
def test_llm_normalize_returns_corrections(mock_run):
    """Successful end-to-end with mocked subprocess."""
    mock_run.return_value = MagicMock(
        stdout='{"corrections": [{"transcribed": "fondos", "correct": "Pandavas"}]}',
        returncode=0,
    )
    result = llm_normalize("The fondos went to war")
    assert len(result) == 1
    assert result[0]["transcribed"] == "fondos"


@patch("normalize.subprocess.run")
def test_llm_normalize_empty_corrections(mock_run):
    """Model returns no corrections."""
    mock_run.return_value = MagicMock(
        stdout='{"corrections": []}',
        returncode=0,
    )
    result = llm_normalize("Once upon a time")
    assert result == []


@patch("normalize.subprocess.run")
def test_llm_normalize_timeout_raises(mock_run):
...
@patch("normalize.subprocess.run")
def test_llm_normalize_default_prompt(mock_run):
    """DEFAULT_PROMPT is used when none provided."""
    mock_run.return_value = MagicMock(
        stdout='{"corrections": []}',
        returncode=0,
    )
    llm_normalize("test text")
...
@patch("normalize.subprocess.run")
def test_llm_normalize_custom_prompt(mock_run):
    ...
    llm_normalize("hello", prompt=custom)
    call_args = mock_run.call_args[0][0]
    assert call_args[3] == "Find errors in: hello"


@patch("normalize.subprocess.run")
def test_llm_normalize_custom_model(mock_run):
    """Model parameter passed to ollama command."""
    mock_run.return_value = MagicMock(
        stdout='{"corrections": []}',
        returncode=0,
    )
    llm_normalize("text", model="llama3:8b")
    call_args = mock_run.call_args[0][0]
